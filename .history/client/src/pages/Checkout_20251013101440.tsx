...
import { validateVoucher } from '../api/voucher';
...
export default function Checkout() {
  ...
  const discountedTotal = appliedVoucher ? Math.round(subtotal * (1 - appliedVoucher.discountPercent / 100)) : subtotal;
            ...
            {cart.items.map((it) => (
              ...
              <div style={{ marginBottom: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <Input value={voucherCode} onChange={(e) => setVoucherCode(e.target.value)} placeholder="Enter voucher code" />
                  <Button onClick={async () => {
                    if (!voucherCode) return message.warning('Please enter a voucher code');
                    try {
                      const res = await validateVoucher(voucherCode);
                      if (res && res.valid) {
                        setAppliedVoucher({ code: res.code, discountPercent: res.discountPercent });
                        message.success(`Voucher ${res.code} applied: ${res.discountPercent}% off`);
                      } else {
                        message.error(res.message || 'Invalid voucher');
                      }
                    } catch (err: any) {
                      console.error('Voucher apply error', err?.response?.data || err);
                      message.error(err?.response?.data?.message || 'Failed to validate voucher');
                    }
                  }}>Apply</Button>
                </div>
                {appliedVoucher && <div style={{ marginTop: 8, color: '#0a8' }}>Applied: {appliedVoucher.code} ({appliedVoucher.discountPercent}% off)</div>}
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>Subtotal <span>{subtotal.toLocaleString()} Đ</span></div>
              {appliedVoucher && <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>Discount ({appliedVoucher.discountPercent}%) <span>-{Math.round(subtotal - discountedTotal).toLocaleString()} Đ</span></div>}
              <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 700 }}>Tổng cộng <span>{discountedTotal.toLocaleString()} Đ</span></div>
          </div>
        </div>
      </div>
    </>
