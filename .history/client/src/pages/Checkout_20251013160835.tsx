...
export default function Checkout() {
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      items: cart.items.map((it: any) => ({ legoId: it.id, quantity: it.quantity, price: it.price })),
    ...
    try {
      ...
      if (payment === 'VNPay') {
        ...
        const demoUrl = `http://sandbox.vnpayment.vn/tryitnow/Home/CreateOrder?orderId=${encodeURIComponent(savedOrder._id)}&amount=${encodeURIComponent(savedOrder.total)}`;
        ...
        Modal.info({
          ...
          content: (
            <div>
              <p>Mở tab VNPay demo, dùng thông tin đơn/amount nếu cần, sau đó dùng các thẻ test từ trang demo.</p>
        ...
        message.success('Đặt hàng thành công. Mã đơn: ' + (savedOrder.orderNumber || savedOrder._id));
      }
