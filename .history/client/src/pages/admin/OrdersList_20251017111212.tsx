...
import { Table, Button, Popconfirm, Tag, notification, message, Tooltip } from 'antd';
  ...
  const columns: ColumnsType<Order> = [
    ...
    { title: 'Created', dataIndex: 'createdAt', key: 'createdAt' },
    {
      ...
      render: (_, record: any) => {
        ...
        // always allow view
        actions.push(
          <Tooltip key="view" title="View order details">
            <Button
              size="small"
              key="view"
              onClick={() => navigate(`/admin/orders/${record._id}`)}
              icon={<EyeOutlined />}
              aria-label={`view-order-${record._id}`}
              data-action="view"
            >
              View
            </Button>
        ...
        if (status === 'pending') {
          actions.push(
            <Popconfirm key="confirm" title="Confirm this order?" onConfirm={() => handleChangeStatus(record._id, 'confirmed')}>
              <Tooltip title="Confirm order" key="confirm-tooltip">
                <Button
                  size="small"
                  type="primary"
                  loading={!!processing[record._id]}
                  aria-label={`confirm-order-${record._id}`}
                  data-action="confirm"
                >
                  Confirm
                </Button>
          ...
          actions.push(
            <Popconfirm key="cancel" title="Cancel this order?" onConfirm={() => handleChangeStatus(record._1d, 'canceled')}>
              <Tooltip title="Cancel order" key="cancel-tooltip">
                <Button
                  danger
                  size="small"
                  loading={!!processing[record._id]}
                  icon={<CloseOutlined />}
                  aria-label={`cancel-order-${record._id}`}
                  data-action="cancel"
                >
                  Cancel
                </Button>
          ...
          actions.push(
            <Popconfirm key="ship" title="Mark as shipped?" onConfirm={() => handleChangeStatus(record._id, 'shipped')}>
              <Tooltip title="Mark order as shipped" key="ship-tooltip">
                <Button
                  size="small"
                  type="default"
                  loading={!!processing[record._id]}
                  icon={<ShoppingOutlined />}
                  aria-label={`ship-order-${record._id}`}
                  data-action="ship"
                >
                  Ship
                </Button>
          ...
          actions.push(
            <Popconfirm key="cancel2" title="Cancel this order?" onConfirm={() => handleChangeStatus(record._id, 'canceled')}>
              <Tooltip title="Cancel order" key="cancel2-tooltip">
                <Button
                  danger
                  size="small"
                  loading={!!processing[record._id]}
          ...
          actions.push(
            <Popconfirm key="deliver" title="Mark as delivered?" onConfirm={() => handleChangeStatus(record._id, 'delivered')}>
              <Tooltip title="Mark order as delivered" key="deliver-tooltip">
                <Button
                  size="small"
                  type="primary"
                  loading={!!processing[record._id]}
                  icon={<SwapRightOutlined />}
                  aria-label={`deliver-order-${record._id}`}
                  data-action="deliver"
                >
                  Deliver
                </Button>
