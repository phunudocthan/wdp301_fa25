...
router.get("/payment-status/:orderId", async (req, res) => {
  try {
    ...
    res.status(200).json({ payment_status: order.payment_status });
  } catch (error) {
    console.error(`🧩 [ERROR] Error fetching payment status for order ${req.params.orderId}:`, error);
    res.status(500).json({ error: "Lỗi server khi kiểm tra trạng thái thanh toán" });
  }
});
