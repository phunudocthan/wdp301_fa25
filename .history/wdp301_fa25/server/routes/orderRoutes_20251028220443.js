...
router.get('/my', requireAuth, async (req, res) => {
  try {
    ...
    // Lấy tất cả đơn TRỪ delivery và delivered
    filter.status = { $nin: ['delivery', 'delivered'] };
    ...
    const items = (itemsRaw || []).map((it) => ({ ...it, user: undefined }));
    res.json({ items, total, page, pageSize: limit });
  } catch (err) {
    console.error('My orders list error', err);
    res.status(500).json({ error: 'Failed to fetch orders' });
  }
});
// GET /api/orders/my - current user's orders (paginated)
router.get('/my-order-history', requireAuth, async (req, res) => {
  try {
    const page = parseInt(req.query.page, 10) || 1;
    const limit = Math.min(parseInt(req.query.limit, 10) || 20, 200);
    const skip = (page - 1) * limit;
    ...
    // Lấy tất cả đơn bao gồm delivery và delivered
    filter.status = { $in: ['delivery', 'delivered'] };
...
// GET /api/orders/:id - order detail (admin or owner)
router.get('/:id/user', requireAuth, async (req, res) => {
  try {
    const id = req.params.id;
    const orderRaw = await Order.findById(id).populate('userId', 'name email').lean();
    if (!orderRaw) return res.status(404).json({ error: 'Order not found' });
  ...
  try {
    ...
    if (!isAdmin) {
      ...
      if (order.status !== 'pending') {
        return res.status(400).json({ error: 'Only pending orders can be canceled by user' });
      }

      // perform cancel + restore stock in a transaction
      const mongoose = require('mongoose');
      const Lego = require('../models/Lego');
      const session = await mongoose.startSession();
      session.startTransaction();
      try {
        ...
        for (const it of order.items) {
          const qty = Number(it.quantity || 0);
          await Lego.findByIdAndUpdate(it.legoId, { $inc: { stock: qty } }, { session });
        }

        // update order status and append history
        const historyEntry = { by: req.user._id, at: new Date(), changes: { status: { from: order.status, to: 'canceled' } } };
        const updatedOrder = await Order.findByIdAndUpdate(id, { $set: { status: 'canceled' }, $push: { history: historyEntry } }, { new: true, session });
        ...
        await session.abortTransaction();
        session.endSession();
        console.error('Transaction error canceling order', txErr);
