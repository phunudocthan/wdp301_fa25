...
router.post("/create_payment_url", async (req, res) => {
  ...
  const amountVnd = Math.round(Number(amount) * usdToVndRate);
  console.log("amountVnd (sau convert):", amountVnd);

  const amountVNPay = amountVnd;
  console.log("vnp_Amount (sau nhân 100):", amountVNPay);
  ...
  const vnp_Params = {
    ...
    vnp_Command: "pay",
    vnp_TmnCode,
    vnp_Amount: amountVnd, // SỐ NGUYÊN VND
    vnp_CurrCode: "VND",
    vnp_TxnRef: orderId,
