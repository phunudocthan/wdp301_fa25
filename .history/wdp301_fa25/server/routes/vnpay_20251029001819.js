...
router.post("/create_payment_url", async (req, res) => {
  let { orderId, amountUSD } = req.body; // luôn gửi USD từ frontend
  ...
  // Chuyển USD -> VND
  const usdToVndRate = 24000; // tỉ giá cố định hoặc lấy động
  const amountVnd = Math.round(amountUSD * usdToVndRate);
  ...
  const vnp_Params = {
    ...
    vnp_Command: "pay",
    vnp_TmnCode,
    vnp_Amount: amountVnd * 100, // VNPay nhân 100
    vnp_CurrCode: "VND",
    vnp_TxnRef: orderId,
