...
router.post("/create_payment_url", async (req, res) => {
  try {
    ...
    const vnp_ReturnUrl = process.env.VNP_RETURNURL; // ví dụ: http://localhost:5000/api/vnpay/return
    ...
    // 🔹 Convert amount sang VND nguyên
    // Nếu amount là USD, quy đổi: const amountVND = Math.round(amount * exchangeRate);
    const amountVND = Math.round(amount)*100; // số nguyên VND
    if (amountVND <= 0) return res.status(400).json({ message: "Invalid amount" });
    ...
    let vnp_Params = {
      ...
      vnp_Amount: amountVND, // VNPay nhân đôi số tiền
      vnp_ReturnUrl,
      vnp_IpAddr: ipAddr,
