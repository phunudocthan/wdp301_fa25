...
// 🔹 Tạo chuỗi chữ ký VNPay
function createVnpSign(params, secret) {
  const sorted = Object.keys(params)
    .sort()
    .reduce((a, k) => ((a[k] = params[k]), a), {});
...
router.post("/create_payment_url", async (req, res) => {
  try {
    ...
    const amountForVNP = (amountVND * 100).toString(); // VNPay expects amount * 100
    ...
    // 🔹 Tạo chữ ký
    const vnp_SecureHash = createVnpSign(vnp_Params, vnp_HashSecret);
