...
router.post("/", requireAuth, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (voucherId) {
        const Voucher = require("../models/Voucher");
        const usedCount = await Order.countDocuments({ voucherId }).session(
          session
        );
        if (usedCount >= appliedVoucher.usageLimit) {
  ...
  try {
    ...
    if (!isAdmin) {
      ...
      if (order.status !== "pending") {
        return res
          .status(400)
          .json({ error: "Only pending orders can be canceled by user" });
      }
      ...
      try {
        ...
        const historyEntry = {
          by: req.user._id,
          at: new Date(),
          changes: { status: { from: order.status, to: "canceled" } },
        };
        const updatedOrder = await Order.findByIdAndUpdate(
          id,
          { $set: { status: "canceled" }, $push: { history: historyEntry } },
          { new: true, session }
        );
    ...
    if (paymentStatus && paymentStatus !== order.paymentStatus) {
      ...
      historyEntry.changes.paymentStatus = {
