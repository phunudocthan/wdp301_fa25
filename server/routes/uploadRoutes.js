...
const uploadReview = require("../utils/multerReviewConfig");
...
/**
 * @desc    Upload review images (used by customers when submitting reviews)
...
router.post(
  ...
  (req, res) => {
    try {
      ...
      res.json({ success: true, message: "Upload ảnh thành công", data: { images: imageUrls } });
    } catch (error) {
...
/**
 * @desc    Upload single review image
...
router.post(
  ...
  (req, res) => {
    try {
      ...
      const imageUrl = `${req.protocol}://${req.get("host")}/uploads/reviews/${req.file.filename}`;
      res.json({ success: true, message: "Upload ảnh thành công", data: { image: imageUrl } });
    } catch (error) {
