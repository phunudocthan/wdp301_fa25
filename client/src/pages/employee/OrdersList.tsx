...
import { Table, Button, Popconfirm, Tag, notification, message } from "antd";
...
const EmployeeOrdersList: React.FC = () => {
    ...
    try {
      ...
      const normalized = items.map((it: any) => ({
        ...
        user:
          ...
          it.userId ||
          it.userId?._id ||
          (typeof it.userId === "object" ? it.userId : it.userId),
      }));
  ...
  const columns: ColumnsType<Order> = [
    {
      ...
      key: "orderNumber",
      render: (text, record) => (
        <a onClick={() => navigate(`/employee/orders/${record._id}`)}>
          {text || record._id}
        </a>
      ),
    },
      ...
      render: (_, record: any) => {
        ...
        actions.push(
          <Button
            size="small"
            key="view"
            onClick={() => navigate(`/employee/orders/${record._id}`)}
            icon={<EyeOutlined />}
          >
            View
          </Button>
        ...
        if (status === "pending") {
          actions.push(
            <Popconfirm
              ...
              onConfirm={() => handleChangeStatus(record._id, "confirmed")}
            >
              <Button
                size="small"
                type="primary"
          ...
          actions.push(
            <Popconfirm
              ...
              onConfirm={() => handleChangeStatus(record._id, "delivered")}
            >
              <Button
                size="small"
                type="primary"
